...
typedef struct ldap_instance {
	...
	char const      *realm;				//!< Realm used for proxying request
 ...
 *	the main log.
 */
#define LDAP_INFO(fmt, ...) INFO("rlm_ldap_forwarder (%s): " fmt, inst->xlat_name, ##__VA_ARGS__)
#define LDAP_WARN(fmt, ...) WARN("rlm_ldap_forwarder (%s): " fmt, inst->xlat_name, ##__VA_ARGS__)

#define LDAP_DBGW(fmt, ...) radlog(L_DBG_WARN, "rlm_ldap_forwarder (%s): " fmt, inst->xlat_name, ##__VA_ARGS__)
#define LDAP_DBGW_REQ(fmt, ...) do { if (request) {RWDEBUG(fmt, ##__VA_ARGS__);} else {LDAP_DBGW(fmt, ##__VA_ARGS__);}} while (0)

#define LDAP_DBG(fmt, ...) radlog(L_DBG, "rlm_ldap_forwarder (%s): " fmt, inst->xlat_name, ##__VA_ARGS__)
#define LDAP_DBG_REQ(fmt, ...) do { if (request) {RDEBUG(fmt, ##__VA_ARGS__);} else {LDAP_DBG(fmt, ##__VA_ARGS__);}} while (0)

#define LDAP_DBG2(fmt, ...) if (debug_flag >= L_DBG_LVL_2) radlog(L_DBG, "rlm_ldap_forwarder (%s): " fmt, inst->xlat_name, ##__VA_ARGS__)
...
#define LDAP_DBG3(fmt, ...) if (debug_flag >= L_DBG_LVL_3) radlog(L_DBG, "rlm_ldap_forwarder (%s): " fmt, inst->xlat_name, ##__VA_ARGS__)
...
#define LDAP_ERR(fmt, ...) ERROR("rlm_ldap_forwarder (%s): " fmt, inst->xlat_name, ##__VA_ARGS__)
