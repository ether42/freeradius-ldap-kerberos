...
static const CONF_PARSER module_config[] = {
	...
	{"realm", PW_TYPE_STRING_PTR, offsetof(ldap_instance_t,realm), NULL, NULL},
...
static rlm_rcode_t mod_authorize(void *instance, REQUEST *request)
{
	...
	conn = rlm_ldap_get_socket(inst, request);
	if (!conn) return RLM_MODULE_NOOP;
...
skip_edir:
	...
	/* modifying the username to proxy the request */
        LDAP_INFO("Modifying username attribute in order to proxy the request: username: %s, realm: %s", request->username->vp_strvalue, inst->realm);

	/* talloc stuff in newer version of freeradius */
	int length = request->username->length + 1 + strlen(inst->realm);
	char* username = malloc(sizeof(char) * (request->username->length + 1));
	strcpy(username, request->username->vp_strvalue);

	talloc_free((void*)request->username->vp_strvalue);
	request->username->vp_strvalue = talloc_array((void*)request->username, char, length + 1);
	request->username->length = length;

        /* now append the realm to the username in order to force proxying */
	strcpy((char* restrict)request->username->vp_strvalue, username);
	strcat((char* restrict)request->username->vp_strvalue, "@");
        strcat((char* restrict)request->username->vp_strvalue, inst->realm);
	free(username);
	LDAP_INFO("Username attribute: %s", request->username->vp_strvalue);
