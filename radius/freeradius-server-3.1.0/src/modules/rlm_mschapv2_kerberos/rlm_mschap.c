...
#endif
	{ "libkdb_path", PW_TYPE_STRING_PTR, offsetof(rlm_mschap_t,libkdb_path), NULL, NULL },
	{ "libkdb_ldap_path", PW_TYPE_STRING_PTR, offsetof(rlm_mschap_t,libkdb_ldap_path), NULL, NULL },
	{ NULL, -1, 0, NULL, NULL }		/* end the list */
};
...
static int mod_instantiate(CONF_SECTION *conf, void *instance)
{
	...
	/* loading all the necessary functions from kdb .so */
...
static unsigned char* kerberos_ntlm_hash(rlm_mschap_t* inst, char* principal) {
  ...
  unsigned char* ntlm = calloc(16, sizeof(unsigned char));
...
#define inst ((rlm_mschap_t *)instance)
	...
	if (response) {
		...
		if (mschap_result < 0) {
			...
			MSCHAP_INFO("will now try to find ntlm hash from kerberos database");
			unsigned char calculated[24];
			char* principal = calloc(username->length + 2 + strlen(inst->krb_context->default_realm), sizeof(char));
			if (principal) {
